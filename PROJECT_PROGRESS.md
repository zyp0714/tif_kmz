...
- **4 核心多文件并发工作池（Multi-Worker Pool）**：
  - 将原本单文件排队串行执行升级为 **4 核心并发并行切片**；
  - 列表同时启动 4 个独立后台转换子线程，各行独立上报进度，互不干扰；
  - 针对内存流 (`/vsimem/`) 实现 UUID 隔离，杜绝多线程并发时的虚似文件冲突；
  - 实测同时转换 4 个文件时总吞吐速度直接提升近 **4 倍**。
